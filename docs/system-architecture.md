...
|---|---|---|---|
| `AltarItemGroupEntity` | Tab palette storefront (vd "Bát hương"). `renderOnAltar=false` = nhóm chỉ hiện danh sách/tổng số lượng, không đặt lên canvas. | — | 8 dòng (7 `renderOnAltar=true` + 1 `false`) |
| `AltarStyleEntity` | Men/tone gốm (Men lam, Men lam vẽ vàng). | — | 2 dòng |
...
| `AltarPlacementEntity` | Vị trí mặc định + overlay PNG trong suốt cho **1 product image cụ thể** (`0..1`, unique trên `product_image_id`). `zIndexOverride=null` = auto-z từ `defaultY`. | `product_image_id` (1:1) | 25 dòng — một placement cho mỗi sản phẩm gốm (`priority = 0` image, decision D5/D7) |
| `AltarPresetEntity` | Bộ gợi ý dựng sẵn (admin author), gắn với đúng 1 `AltarModelSize`, style optional. | `altar_model_size_id`, `altar_style_id?` | 3 dòng (127cm men-lam, 153cm men-lam, 175cm men-lam-vẽ-vàng) |
| `AltarPresetItemEntity` | 1 item trong preset — 2 dạng phân biệt bằng nullability: canvas item (`productImage`+`x`+`y` đều set) hoặc accessory (`productImage`/`x`/`y` đều null, chỉ `quantity`). | `preset_id`, `product_id`, `product_image_id?` | 28 dòng (7 + 12 + 9 qua 3 preset) |
...
`ProductEntity` có thêm 2 FK optional `altarItemGroup`/`altarStyle` (gán cho 28 sản phẩm `BO_DO_THO` thuộc bộ altar-customizer seed ở `ProductSeeder` — 25 sản phẩm gốm có cả hai FK, 3 phụ kiện phi gốm chỉ có `altarItemGroup`; mọi sản phẩm/category khác giữ `null`).

**Bộ dữ liệu seed (catalog ảnh thật, xây lại từ `tasks/seeder-alter-customize`):** 105 ảnh PNG trong suốt thật (đã crop/resize/nén PNG-8, tổng ~11.4MB) cho 25 sản phẩm gốm Bát Tràng, lưu tại `vu-gia-client/public/assets/images/altar-customizer/products/<slug>/01..NN.png` (ảnh `01.png` = overlay đặt canvas, decision D5), cùng convention bare-relative-path với `ProductCategorySeeder`. 3 phụ kiện phi gốm mượn 1 ảnh sản phẩm gốm sẵn có làm thumb (decision D3). Mọi sản phẩm gốm đều có placement thật — không còn scope reduction "chỉ 1 nhóm kéo-thả được" như trước.

**`ProductSeeder`'s `buildProducts()` — rủi ro thứ tự vị trí (positional index risk).** Combo #12 (index 11) resolve 3 sub-product bằng `saved.get(8/9/10)` theo **vị trí trong list**, không phải id. 28 sản phẩm altar-set được **append sau index 11** (không chèn giữa danh sách gốc), giữ nguyên 100% logic/index đã test của combo — lựa chọn rủi ro thấp hơn so với refactor toàn bộ sang tra cứu theo slug (phương án còn lại plan có đề xuất). FK altar item group/style, và catalog dữ liệu altar (`AltarProductCatalog`), dùng map/list tra theo `slug` (code mới, không có hành vi cũ cần bảo toàn). `ProductRepository.findAltarCustomizerItems` lọc thêm `aig.id IS NOT NULL` để loại 5 sản phẩm `BO_DO_THO` gốc không có altar item group khỏi feed customizer.
